...
class EconomyCog(commands.Cog):
    def __init__(self, bot):
        ...
        self.recent_messages      = {}   # {guild_id: [timestamps]}
    ...
    async def contador_economia_loop(self):
        ...
        # Verifica cada guild que tenha mensagens recentes
        for guild in self.bot.guilds:
            canal_id = get_config(guild.id, "canal_economia")
            if not canal_id:
                continue

            msgs = self.recent_messages.get(guild.id, [])
            msgs = [ts for ts in msgs if ts >= limite]
            self.recent_messages[guild.id] = msgs
    ...
    async def on_message(self, message: discord.Message):
        ...
        if message.channel.id == canal_id:
            msgs = self.recent_messages.setdefault(message.guild.id, [])
            msgs.append(datetime.now(timezone.utc))
            ...
            if self.economy_can_win and self.economy_active_text is not None:
                if message.content.strip().lower() == self.economy_active_text.lower():
                    self.economy_can_win     = False
                    self.economy_active_text = None
                    UserManager.adicionar_flingers(message.author.id, message.guild.id, self.economy_reward)
    ...
    async def toggle_gerador(self, interaction: discord.Interaction):
        ...
        if self.gerador_ativo:
            await interaction.response.send_message(
                ...
                "O gerador verificará a cada 20 minutos se há atividade suficiente.", ephemeral=True
            )
        else:
            await interaction.response.send_message(
